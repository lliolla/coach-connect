...
const SortableExerciseCard = ({
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex gap-2 items-center mt-1">
              <Badge
                variant="secondary"
                className="text-[9px] uppercase px-1.5 py-0 h-4"
              >
                {ex.category}
              </Badge>

              {ex.description && (
                <span className="text-[10px] text-muted-foreground italic truncate max-w-[180px] sm:max-w-[300px]">
                  {ex.description}
                </span>
              )}
            </div>
      ...
      <div
        className="grid grid-cols-2 gap-3 mb-3 relative z-10"
      ...
      >
        <div className="space-y-1">
          ...
          <Select
            value={String(ex.rest_time_seconds ?? 60)}
            onValueChange={(v) =>
              updateExerciseDetails(
                index,
                "rest_time_seconds",
          ...
          >
            <SelectTrigger className="h-9 text-sm font-bold bg-background border-primary/10 focus:border-primary">
              <SelectValue placeholder="Repos" />
            </SelectTrigger>

            <SelectContent>
              {[0, 15, 30, 45, 60, 90, 120, 180, 240, 300].map(
                (s) => (
                  <SelectItem
                    key={s}
                    value={String(s)}
...
export const CardSeance = ({
  ...
  const context =
    searchParams.get("context") || "seances"

  const effectiveIsTracking =
    isTracking || context === "seances"

  const isCreation = !seanceId
  const isView = mode === "view"
  const isDuplicate = mode === "duplicate"

  const [openExercises, setOpenExercises] =
    React.useState({
      open: false,
      section: "main",
    })
  ...
  const [showMaxSessionsModal, setShowMaxSessionsModal] =
    React.useState(false)

  const [objectifsList, setObjectifsList] =
    React.useState([])

  const [formData, setFormData] =
    React.useState(initialFormState)

  const [isManualDuration, setIsManualDuration] =
    React.useState(false)

  const [availableExercises, setAvailableExercises] =
    React.useState([])

  const [availableAthletes, setAvailableAthletes] =
    React.useState([])

  const [availableObjectifs, setAvailableObjectifs] =
    React.useState([])
  ...
  const fetchSeance = async (idToFetch) => {
    try {
      ...
      const data =
        await getSessionById(idToFetch)
      ...
      setFormData({
        ...
        athlete_id: isDuplicate
          ? null
          : data.athlete_id != null
            ? String(data.athlete_id)
            : null,

        objectif_id: isDuplicate
          ? null
          : data.objectif_id != null
            ? String(data.objectif_id)
            : null,
      ...
      if (
        mappedExercises.some(
          (ex) => ex.section === "warmup"
        )
      ) {
        sectionsToExpand.warmup = true
      }
      ...
      if (
        ...
        sectionsToExpand.main = true
      }

      if (
        mappedExercises.some(
          (ex) => ex.section === "cooldown"
        )
      ) {
        sectionsToExpand.cooldown = true
      }
      ...
      toast.error(
        "Impossible de charger les détails de la séance"
      )
    } finally {
      setLoading(false)
    }
  ...
  const fetchAthletes = async () => {
    try {
      ...
      toast.error(
        "Impossible de charger la liste des athlètes"
      )
    }
  }


  const fetchObjectifsList =
    async () => {
      try {
        const data =
          await getObjectifs()
  ...
  const addExerciseToSeance =
    (
      ...
      toast.success(
        `${exerciseTemplate.name} ajouté`
      )
    }


  const requestRemoveExercise =
    (index) => {
  ...
  const handleSubmit = async () => {
    ...
    /* ==========================================================
       VERIFICATION LIMITE OBJECTIF

       IMPORTANT :
       On utilise maintenant isObjectifFull
       depuis @/app/actions/sessions

       On ne passe PLUS par :
       /api/objectifs/[id]/check-full
    ========================================================== */
    ...
    if (
      ...
      try {
        console.log(
          "Vérification limite objectif :",
          formData.objectif_id
        )

        const isFull =
          await isObjectifFull(
            formData.objectif_id
          )
        ...
        console.error(
          "Erreur lors de la vérification de l'objectif :",
          error
        )

        toast.error(
          "Impossible de vérifier le nombre de séances de l'objectif"
        )
  ...
  return (
    <>
      <Card className="w-full max-w-3xl mx-auto border shadow-none relative bg-background/50 backdrop-blur-sm">
        ...
        <CardHeader className="flex flex-row items-center justify-between border-b bg-muted/5 pb-6">
          ...
          <Button
            ...
            onClick={() =>
              router.push(
                formData.athlete_id
                  ? `/admin/users/${formData.athlete_id}`
                  : context === "seances"
                    ? "/admin/seances"
                    : "/admin/modeles"
              )
        ...
        <CardContent className="space-y-8 pt-6">
          ...
          <div className="space-y-4">
            ...
            <div
              className="flex items-center justify-between border-b pb-2 cursor-pointer group select-none"
              onClick={() =>
                toggleSection("info")
              }
            >
            ...
            {expandedSections.info && (
              <div className="space-y-6">
                          ...
                          .map(
                            (obj) => (
                              ...
                              >
                                ...
                                {obj.total_sessions &&
                                  ` (${obj.sessions?.length || 0}/${obj.total_sessions})`}
                              </SelectItem>
                            )
                          )}
                ...
                <div className="space-y-2">
                  ...
                  <Label className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">
                    Description globale (Facultatif)
                  </Label>
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="transmis">
                          Transmis
                        </SelectItem>

                        <SelectItem value="prévu">
                          Prévu
                        </SelectItem>
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            ...
            {isView ? (
              <>
                ...
                <Button
                  className="flex-1 sm:px-10 h-12 rounded-xl font-bold uppercase tracking-widest text-xs"
                  onClick={() =>
                    router.push(
                      formData.athlete_id
                        ? `/admin/users/${formData.athlete_id}`
              ...
              <Button
                className="flex-1 sm:px-10 h-12 rounded-xl font-bold uppercase tracking-widest text-xs shadow-lg shadow-primary/20 active:scale-[0.98] transition-all"
      ...
      >
        ...
        <DialogContent className="sm:max-w-xl p-0 overflow-hidden">
          ...
          <div className="p-6 border-b bg-muted/5">
            ...
            <DialogDescription>
              Sélectionnez un exercice pour la section{" "}
              <span className="text-primary font-bold">
                {openExercises.section === "warmup"
                  ? "Échauffement"
                  : openExercises.section === "cooldown"
                    ? "Retour au calme"
                    : "Corps de séance"}
              </span>
            </DialogDescription>
                  ...
                  (ex) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between w-full">
                        ...
                        <Plus
                          size={16}
                          className="text-primary opacity-20 group-hover:opacity-100"
                        />
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogHeader>
            ...
            <DialogDescription className="py-4">
              Êtes-vous sûr de vouloir retirer cet exercice de la séance ?
            </DialogDescription>
      ...
      {/* =========================================================
          MODAL LIMITE OBJECTIF
      ========================================================== */}

      <Dialog
        open={showMaxSessionsModal}
        onOpenChange={
          setShowMaxSessionsModal
        }
      >
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <DialogHeader className="flex flex-col items-center justify-center text-center">
            ...
            <DialogDescription className="text-base py-2">
              ...
              {formData.objectif_id && (
                <>
                  Vous avez atteint le nombre maximal de séances{" "}
                  (
                  {
                    objectifsList.find(
                      (obj) =>
                  ...
                  Souhaitez-vous modifier l'objectif ?
                </>
              )}
          ...
          <DialogFooter className="flex sm:justify-center gap-2">
            ...
            <Button
              ...
              onClick={() => {
                ...
                router.push(
                  `/admin/objectifs/${formData.objectif_id}/edit`
                )
              }}
